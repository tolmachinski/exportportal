        ...
        api.forEach(function(entry) {
            if(groupEntry === entry.group) {
                ...
                if (apiProject.url) {
                    ...
                    for (var ex_key in fields.article.examples) {
                        var obj = fields.article.examples[ex_key];
                        obj.content = obj.content.replace('{{api_host_url}}', apiProject.url);
                    }
                    for (var success_ex_key in entry.success.examples) {
                        var obj = entry.success.examples[success_ex_key];
                        obj.content = obj.content.split('{{api_host_url}}').join(apiProject.url);
                    }
                }
