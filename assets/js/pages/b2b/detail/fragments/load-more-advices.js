...
/**
 * It loads more advices of a request
 * @param {JQuery} btn - load more btn
 */
const loadMoreAdvices = async btn => {
    const list = $("#js-b2b-request-advices-wrapper");
    let itemsLength = list.find(".js-b2b-request-advices-item").length;
    ...
    try {
        const { mess_type: messageType, content, totalCount } = await postRequest(
            `${SITE_URL}b2b/ajax_b2b_operation/more_advices`,
        ...
        if (messageType === "success" && content) {
            list.append(content);
            itemsLength = list.find(".js-b2b-request-advices-item").length;
...
export default loadMoreAdvices;
