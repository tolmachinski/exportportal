...
/**
 * It loads more followers of a request
 * @param {JQuery} btn - load more btn
 */
const loadMoreFollowers = async btn => {
    const list = $("#js-b2b-request-followers-wrapper");
    let itemsLength = list.find(".js-b2b-request-followers-item").length;
    ...
    try {
        const { mess_type: messageType, content, totalCount } = await postRequest(
            `${SITE_URL}b2b/ajax_b2b_operation/more_followers`,
        ...
        if (messageType === "success" && content) {
            list.append(content);
            itemsLength = list.find(".js-b2b-request-followers-item").length;
...
export default loadMoreFollowers;
