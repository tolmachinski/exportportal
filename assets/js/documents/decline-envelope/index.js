 ...
 * @param {JQuery} form
 */
const declineEnvelope = async (url, envelopeId, container, form) => {
    ...
    return postRequest(url, entities.concat(form.serializeArray()), "json")
        .then(data => {
            ...
            if (data.mess_type === "success") {
                ...
                EventHub.trigger("documents:envelope-declined", data.envelope || {});
            }
        })
...
export default async (envelopeId, selectors, sendUrl) => {
    ...
    EventHub.off("documents:decline-envelope");
    EventHub.on("documents:decline-envelope", (e, form) => declineEnvelope(new URL(sendUrl), envelopeId, container, form));
};
