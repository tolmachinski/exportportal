 ...
 * @param {JQuery} form
 */
const signEnvelope = async (url, envelopeId, container, form) => {
    ...
    return postRequest(url, entities.concat(form.serializeArray()), "json")
        .then(data => {
            ...
            if (data.mess_type === "success") {
                ...
                EventHub.trigger("documents:envelope-signed", data.envelope || {});
            }
        })
...
export default async (envelopeId, selectors, sendUrl) => {
    ...
    EventHub.off("documents:sign-envelope");
    EventHub.on("documents:sign-envelope", (e, form) => signEnvelope(new URL(sendUrl), envelopeId, container, form));
};
