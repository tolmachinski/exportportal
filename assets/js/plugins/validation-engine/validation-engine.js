...
const methods = {
    ...
    attach: function (userOptions) {
        ...
        if (options.binded) {
            ...
            form.on(
                ...
                "[" + options.validateAttribute + "*=validate]:not([type=checkbox]):not([type=radio]):not(.js-datepicker-validate)",
            ...
            form.on(
                options.validationEventTrigger,
                "[" + options.validateAttribute + "*=validate][class*=js-datepicker-validate]",
                { delay: 300 },
                methods._onFieldEvent
            );
        }
    ...
    detach: function () {
        ...
        form.off(options.validationEventTrigger, "[" + options.validateAttribute + "*=validate][class*=js-datepicker-validate]", methods._onFieldEvent);
    ...
    updatePromptsPosition: function (event) {
        ...
        form.find("[" + options.validateAttribute + "*=validate]")
            ...
            .each(function () {
                ...
                var promptText = $(prompt).find(".formErrorContentTxt").html();
    ...
    _onFieldEvent: function (event) {
        ...
        if (field.hasClass("select2-container--disabled")) {
            return false;
        }
        ...
        if (field.hasClass("select2") && !field.hasClass("select2-container--disabled")) {
            // field.attr('tabindex', 0);
            field
                .find(".select2-selection")
                .on("focus", function () {
                    var $fieldError = field.prev(".formError");
                    $fieldError.addClass("show").removeClass("hide");
                    methods._updateThisPrompt(field, options, $fieldError);
                })
                .on("blur", function () {
                    field.prev(".formError").addClass("hide").removeClass("show");
                });
    ...
    _validateFields: function (form) {
        ...
        if (errorFound) {
            if (options.scroll) {
                ...
                //prompt positioning adjustment support. Usage: positionType:Xshift,Yshift (for e.g.: bottomLeft:+20 or bottomLeft:-20,+10)
    ...
    _validateField: function (field, options, skipAjaxValidation) {
        ...
        field.trigger("jqv.field.validating", [field, options, form]);
        ...
        for (var i = 0, field_errors = 0; i < rules.length; i++) {
            ...
            var isAjaxRule = false;
            var currentRule = rules[i];
            var isValidRuleType = true;
            if (["custom", "ajax", "funcCall"].indexOf(currentRule) !== -1) {
                currentRule = rules[i + 1] || null;
            }

            field.trigger("jqv.field.rule.checking", [field, currentRule, rules, options, form]);
            ...
            if (currentRule && isValidRuleType && !isAjaxRule) {
                field.trigger("jqv.field.rule.checked", [field, options.isError, currentRule, rules, options, form]);
            }
        }
        ...
        if (!required && !field && !field.val() && field.val().length < 1 && $.inArray("equals", rules) < 0) options.isError = false;
        ...
        if (!isAjaxValidator) {
            field.trigger("jqv.field.validated", [field, options.isError, options, form]);
     ...
     * @param {number} i rules index
     * @param {any}
     *            user options
     * @param {boolean} condRequired flag when method is used for internal purpose in condRequired check
     ...
     * Checks date range
     *
     * @param {JQuery} first field name
     * @param {JQuery} second field name
     * @return an error string if validation failed
     */
    ...
    _ajax: function (field, rules, i, options) {
        var form = $(field.prop("form") || field.closest("form, .validationEngineContainer"));
        ...
        if (!options.isError && !methods._checkAjaxFieldStatus(field.attr("id"), options)) {
            $.ajax({
                ...
                success: function (json) {
                    ...
                    errorField.trigger("jqv.field.rule.checked", [errorField, options.isError, errorSelector, rules, options, form]);
                    errorField.trigger("jqv.field.validated", [errorField, options.isError, options, form]);
    ...
    _buildPrompt: function (field, promptText, type, ajaxed, options) {
        ...
        var promptTtl = "";
        if (field.data("label")) {
            promptTtl = '<div class="formErrorTtl"><span class="txt">' + field.data("label") + "</span></div>";
        }

        var promptContent = $("<div>")
            .addClass("formErrorContent")
            .html(promptTtl + '<div class="formErrorContentTxt">' + promptText + "</div>")
            .appendTo(prompt);
        ...
        if (requiredOverride !== undefined) {
            ...
            if (options.prettySelect) {
                if (
                    ...
                    var prettyOverrideClass = $("#" + field.attr("id").substr(options.usePrefix.length).substring(options.useSuffix.length)).attr(
                        "data-required-class"
                    );
    ...
    _updateThisPrompt: function (field, options, fieldError) {
        methods._updatePrompt(field, fieldError, fieldError.find(".formErrorContentTxt").html(), undefined, false, options);
    },
    /**
    ...
    _updatePrompt: function (field, prompt, promptText, type, ajaxed, options, noAnimation) {
        if (prompt) {
            ...
            prompt.find(".formErrorContentTxt").html(promptText);
            ...
            if ($("body").hasClass("rtl")) {
                ...
                if (prompt.is(":visible")) {
                    css["margin-top"] = pos.marginTopSize;
                }
            }
    ...
    _closePrompt: function (field) {
        var prompt = methods._getPrompt(field);
        if (prompt)
            // Fix bug with double clicks on validation checkbox
     ...
     * @param {JQuery}
     *            field
     * @return undefined or the error prompt (JQuery)
     */
    ...
    _calculatePosition: function (field, promptElmt, options) {
        if (promptElmt.css("opacity") === "0") {
            promptElmt.css("top", "initial");
            promptElmt.css("left", "initial");
        }
...
export default async function () {
    ...
    // LEAK GLOBAL OPTIONS
    $.extend($, {
        validationEngine: defaultOptions,
        validationEngineLanguage: {
            newLang: async () => {
                try {
                    await import(/* webpackChunkName: "validation-engine-i18n" */ `@plug/jquery-validation-engine-2-6-2/lang/${LANG}.js`);
                } catch (error) {
                    // Sadly - nothing
                }

                $["validationEngineLanguage"].allRules = i18nDomain({ plug: "validationEngine" });
            },
        },
    });

    await $['validationEngineLanguage'].newLang();
    defaultOptions.defaults.promptPosition = methods.isRTL()
        ? "topLeft"
        : "topRight";
}
