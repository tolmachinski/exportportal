...
use App\Messenger\Message\Command\Matrix\UpdateServerNoticesRoom;
use App\Messenger\Message\Event\Matrix\UserServerNoticesRoomUpdatedEvent;
 ...
 */
final class UpdateServerNoticesRoomHandler implements MessageSubscriberInterface, LoggerAwareInterface
{
     ...
     * Handles the message.
     */
    public function __invoke(UpdateServerNoticesRoom $message): void
    {
        ...
        if (null === $userReference['server_notices_room_id']) {
            // Silently fail.
            if ($this->logger) {
                $this->logger->warning(sprintf('The the server notice room for user with ID "%d" is not yet created in the sync table.', $userId), [
                    'userId'  => $userId,
                    'message' => $message,
                ]);
            }
        ...
        try {
            ...
            foreach ($states as $state) {
                $roomApi->setRoomStateWithKey($userReference['server_notices_room_id'], $state->getType(), '', $state->getContent());
            }
        ...
        $this->eventBus->dispatch(new Envelope(
            new UserServerNoticesRoomUpdatedEvent((int) $userReference['id_user'], $userReference['server_notices_room_id']),
    ...
    {
        yield UpdateServerNoticesRoom::class => ['bus' => 'command.bus'];
    }
    ...
    {
        $states = [
            (new StateEvent())->setType((string) StateEventType::NAME())->setContent((object) ['name' => \translate('matrix_chat_server_notices_room_display_name')]),
            (new StateEvent())->setType((string) StateEventType::TOPIC())->setContent((object) ['topic' => \translate('matrix_chat_server_notices_room_topic')]),
        ];

        if (null !== ($avatarPath = \config('matrix_server_notices_room_avatar_path') ?? null)) {
