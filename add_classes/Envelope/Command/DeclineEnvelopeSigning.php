...
use App\Envelope\Message\DeclineEnvelopeSigningMessage;
...
final class DeclineEnvelopeSigning implements CommandInterface
{
     ...
     * {@inheritdoc}
     */
    public function __invoke(DeclineEnvelopeSigningMessage $message)
    {
        ...
        try {
            ...
            $this->addHistoryEvent(new HistoryEvent(HistoryEvent::DECLINE), $envelopeId, $message->getSenderId());
            ...
            if (isset($envelope['id_sender'])) {
                $this->notifier->send(
                    new OrderDocumentEnvelopeNotification(Type::DECLINED_ENVELOPE, $orderId, $envelopeId, $envelope['display_title'], $sender, [
            ...
            if (!empty($message->getAccessRulesList())) {
                $this->notifier->send(
                    new OrderDocumentEnvelopeNotification(Type::DECLINED_ENVELOPE_FOR_MANAGER, $orderId, $envelopeId, $envelope['display_title'], $sender, [
                        (string) SystemChannel::STORAGE(),
                    ]),
