...
final class DownloadLatestActionButton extends ActionButton
{
    /**
    ...
    public function acceptsRow(array $envelope): bool
    {
        if (!$this->ignoreStatus) {
            ...
            if (\in_array($envelope['status'], [EnvelopeStatuses::VOIDED, EnvelopeStatuses::CREATED])) {
                return false;
            }
        }

        if (empty($envelope['documents']['latest'])) {
            return false;
        }
