...
use Ep_Events_Speakers_Model;
use Symfony\Component\HttpFoundation\ParameterBag;

class WebinarValidator extends Validator
{
    ...
    protected function rules(): array
    {
        ...
        return [
            ...
            'speaker' => [
                'field' => $fields->get('speaker'),
                'label' => $labels->get('speaker'),
                'rules' => $this->getSpeakerRules($messages),
            ],
            'title' => [
            ...
            'link' => [
                ...
                'rules' => $this->getLinkRules($messages),
            ],
        ];
    }
    ...
    {
        return [
            'speaker.wrongDataType' => translate('validation_ep_webinar_speaker_wrong_data', ['{{FIELD_NAME}}' => '%s']),
            'tags.notAllAreValid'   => translate('validation_ep_webinar_tags_not_valid'),
            'category.notNumber'    => translate('validation_ep_webinar_category_id_not_number', ['{{FIELD_NAME}}' => '%s']),
            'gallery.wrongData'     => translate('validation_ep_webinar_gallery_wrong_data', ['{{FIELD_NAME}}' => '%s']),
            'speaker.notFound'      => translate('validation_ep_webinar_speaker_not_exist'),
            'category.exist'        => translate('validation_ep_webinar_category_not_exist', ['{{FIELD_NAME}}' => '%s']),
            'gallery.limit'         => translate('validation_ep_webinar_gallery_limit_exceeded', ['{{IMAGES_LIMIT}}' => '%d']),
        ];
    }
    ...
    /**
     * Get the link validation rules.
     */
    protected function getLinkRules(ParameterBag $messages): array
    {
        return [
            'valid_url' => $messages->get('link.maxLength') ?? '',
            'required'  => $messages->get('link.required') ?? '',
        ];
    }
     ...
     * Get the dateStart field validation rules.
     */
    protected function getSpeakerRules(ParameterBag $messages): array
    {
        return [
            'is_natural_no_zero'    => $messages->get('speaker.wrongDataType') ?? '',
            function (string $attr, $speakerId, callable $fail, TinyMVC_Library_validator $validator) use ($messages) {
                /** @var Ep_Events_Speakers_Model $eventSpeakersModel */
                $eventSpeakersModel = model(Ep_Events_Speakers_Model::class);
                if (!$eventSpeakersModel->has((int) $speakerId)) {
                    $fail($messages->get('speaker.notFound'));
                }
            },
        ];
    }
