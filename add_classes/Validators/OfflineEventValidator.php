...
use Ep_Events_Partners_Model;
use Symfony\Component\HttpFoundation\ParameterBag;

class OfflineEventValidator extends Validator
{
    ...
    protected function rules(): array
    {
        ...
        return [
            'partners' => [
                'field' => $fields->get('partners'),
                'label' => $labels->get('partners'),
                'rules' => $this->getPartnersRules($messages),
            ],
            'nrOfParticipants' => [
                'field' => $fields->get('nrOfParticipants'),
                'label' => $labels->get('nrOfParticipants'),
                'rules' => $this->getNrOfParticipantsRules($messages),
            ],
            'ticketPrice' => [
                'field' => $fields->get('ticketPrice'),
                'label' => $labels->get('ticketPrice'),
                'rules' => $this->getTicketPriceRules($messages),
            ],
            'dateEnd' => [
    ...
    protected function fields(): array
    {
        return [
            ...
            'nrOfParticipants'  => 'nr_of_participants',
    ...
    protected function labels(): array
    {
        return [
            ...
            'nrOfParticipants'  => 'Number of participants',
    ...
    {
        return [
            'ticketPrice.positiveNumber'    => translate('validation_offline_ep_event_ticket_price_positive_number', ['{{FIELD_NAME}}' => '%s']),
            'tags.notAllAreValid'           => translate('validation_offline_ep_event_tags_not_valid'),
            'category.notNumber'            => translate('validation_offline_ep_event_category_id_not_number', ['{{FIELD_NAME}}' => '%s']),
            'partners.wrongData'            => translate('validation_offline_ep_event_partners_wrong_data', ['{{FIELD_NAME}}' => '%s']),
            'gallery.wrongData'             => translate('validation_offline_ep_event_gallery_wrong_data', ['{{FIELD_NAME}}' => '%s']),
            'category.exist'                => translate('validation_offline_ep_event_category_not_exist', ['{{FIELD_NAME}}' => '%s']),
            'gallery.limit'                 => translate('validation_offline_ep_event_gallery_limit_exceeded', ['{{IMAGES_LIMIT}}' => '%d']),
        ];
    }
    ...
    /**
     * Get the ticket price field validation rules.
     */
    protected function getTicketPriceRules(ParameterBag $messages): array
    {
        return [
            'required'          => $messages->get('ticketPrice.required') ?? '',
            function (string $attr, $value, callable $fail) use ($messages) {
                if (empty($value)) {
                    return;
                }

                if (! (bool) filter_var($value, FILTER_VALIDATE_FLOAT, ['options' => ['min_range' => 0]])) {
                    $fail(sprintf($messages->get('ticketPrice.positiveNumber'), $attr));
                }
            }
        ];
    }
    ...
    /**
     * Get the nr of participants field validation rules.
     */
    protected function getNrOfParticipantsRules(ParameterBag $messages): array
    {
        return [
            'required'              => $messages->get('nrOfParticipants.required') ?? '',
            'is_natural_no_zero'    => $messages->get('nrOfParticipants.undefinedType') ?? '',
        ];
    }
    ...
    protected function getGalleryRules(ParameterBag $messages): array
    {
        return [
            function (string $attr, $gallery, callable $fail) use ($messages) {
                ...
                $limit = config('img.ep_events.gallery.limit', 1);
                if ($gallery->count() > $limit) {
                    $fail(sprintf($messages->get('gallery.limit'), $limit));
                    return;
                }
            }
        ];
    ...
    /**
     * Get the partners field validation rules.
     */
    protected function getPartnersRules(ParameterBag $messages): array
    {
        return [
            function (string $attr, $partners, callable $fail) use ($messages) {
                if (empty($partners)) {
                    return;
                }

                if (!$partners instanceof NestedValidationData) {
                    $fail(sprintf($messages->get('partners.wrongData'), $attr));
                    return;
                }

                $partnersIds = iterator_to_array($partners->getIterator());

                foreach ($partnersIds as $partnerId) {
                ...
                if ($partners->count() != $eventPartnersModel->countBy([
                    'conditions' => [
