 ...
 */
class UpdateServerNoticesRoom extends MatrixCommand
{
    /**
     * {@inheritdoc}
     */
    protected $name = 'matrix:rooms:server-notices:update';

    /**
     * {@inheritdoc}
     */
    protected $description = 'The command that updates the server notices notification room for exsisting users.';

    /**
     * {@inheritdoc}
     */
    protected $help = <<<'HELP'
    The <info>%command.name%</info> command updates the server notices Notification rooms for the Matrix users.
        ...
        try {
            ...
            foreach ($users as $user) {
                $this->messageBus->dispatch(new Envelope(
                    new Commands\UpdateServerNoticesRoom((int) $user['idu']),
            ...
            'exists'  => [
                $this->usersRepository->getRelationsRuleBuilder()->whereHas(
                    ...
                    function (QueryBuilder $query, RelationInterface $relation) {
                        ...
                        $relation->getRelated()->getScope('hasServerNoticesRoom')($query, true);
                    }
                ),
            ],
        ]);
