 ...
 */
class CreateServerNoticesRoom extends MatrixCommand
{
    /**
     * {@inheritdoc}
     */
    protected $name = 'matrix:rooms:server-notices:create';

    /**
     * {@inheritdoc}
     */
    protected $description = 'The command that creates the server notices notification room for exsisting users.';

    /**
     * {@inheritdoc}
     */
    protected $help = <<<'HELP'
    The <info>%command.name%</info> command creates the server notices Notification rooms for the Matrix users.

        <info>php %command.full_name% <user-id></info>

    To create rooms for multiple users at once, pass each user ID:

        <info>php %command.full_name% <user-id-1> <user-id-2></info>

    Use the --all option to create rooms for all users from the application:

        <info>php %command.full_name% --all</info>

        <info>php %command.full_name% -a</info>

    Use the --groups option to create rooms only for users that belongs to the certain group (you must use group ID here):

        <info>php %command.full_name% --all --group=1 --group=2</info>

        <info>php %command.full_name% --all -G 1 -G 2</info>

    Use the --group-alias option to create rooms only for users that belongs to the certain group (you must use group alias here):
    ...
    Use the --status option to create rooms only for users that have a certain status:

        <info>php %command.full_name% --all --status=new --status=pending</info>

        <info>php %command.full_name% --all -S "new" -S "pending"</info>

    Use the --delay option to indicate the interval between create commands. The interval is measured in milliseconds:

        <info>php %command.full_name% <user-id-1> <user-id-2> --delay=7500</info>

        <info>php %command.full_name% <user-id-1> <user-id-2> -d 7500</info>

    Use the --limit option to indicate the limit of users in the create queue:
    ...
    {
        // Fetch users
        $users = $this->findPendingUsers($input);
        if (empty($users)) {
            $output->success('Nothing to create.');
        ...
        try {
            ...
            foreach ($users as $user) {
                $this->messageBus->dispatch(new Envelope(
                    new CreateMatrixServerNoticesRoom((int) $user['idu']),
     ...
     */
    protected function findPendingUsers(InputInterface $input): array
    {
            ...
            'exists'  => [
                $this->usersRepository->getRelationsRuleBuilder()->whereHas(
                    ...
                    function (QueryBuilder $query, RelationInterface $relation) {
                        ...
                        $relation->getRelated()->getScope('hasServerNoticesRoom')($query, false);
                    }
                ),
            ],
        ]);
