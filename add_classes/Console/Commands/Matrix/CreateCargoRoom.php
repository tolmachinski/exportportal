 ...
 */
class CreateCargoRoom extends MatrixCommand
{
    /**
     * {@inheritdoc}
     */
    protected $name = 'matrix:rooms:cargo:create';

    /**
     * {@inheritdoc}
     */
    protected $description = 'The command that creates the Cargo notification room for exsisting users.';

    /**
     * {@inheritdoc}
     */
    protected $help = <<<'HELP'
    The <info>%command.name%</info> command creates the Cargo Notification rooms for the Matrix users.
        ...
        try {
            ...
            foreach ($users as $user) {
                $this->messageBus->dispatch(new Envelope(
                    new Commands\CreateCargoRoom((int) $user['idu']),
            ...
            'exists'  => [
                $this->usersRepository->getRelationsRuleBuilder()->whereHas(
                    ...
                    function (QueryBuilder $query, RelationInterface $relation) {
                        ...
                        $relation->getRelated()->getScope('hasCargoRoom')($query, false);
                    }
                ),
            ],
        ]);
