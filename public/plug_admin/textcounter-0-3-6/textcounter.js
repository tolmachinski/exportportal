...
$.textcounter = function(el, options) {
	...
	// Rewrite options buy dataset
	options.max = base.$el.data('max') ? base.$el.data('max') : $.textcounter.defaultOptions.max;
	options.min = base.$el.data('min') ? base.$el.data('min') : $.textcounter.defaultOptions.min;
	...
	var counterText = base.options.countDown ? base.options.countDownTextBefore : base.options.counterText,
		counterTextAfter = base.options.countDown ? base.options.countDownTextAfter : '',
		counterNum = base.options.countDown ? base.options.max : 0;

	base.$el.after('<' + base.options.countContainerElement + ' class="' + base.options.countContainerClass + '">' + counterText + '<span class="' + base.options.textCountClass + '">' + counterNum + '</span> ' + counterTextAfter + '</' + base.options.countContainerElement + '>');
...
$.textcounter.defaultOptions = {
	...
	'countContainerClass'       : "textcounter-wrapper",     // class applied to the countContainerElement
	'textCountClass'            : "textcounter",             // class applied to the counter length
	...
	'countDownTextBefore'       : "Remaining: ",            // count down text
	'countDownTextAfter'        : "characters",            // count down text
