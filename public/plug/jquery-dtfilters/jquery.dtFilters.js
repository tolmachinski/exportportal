(function( $ ) { //dataTables filters
	...
	var dtFilters = $.fn.dtFilters = function(selector, options) {
		var returnObj = {};
		...
		var methods = {
			...
			activateFilter: function(filter){
				...
				var $li = $ulFilters.find('li.af-' + filter.name);
				if (!$li.length){
					$li = $('<li class="dt-filter__param af-' + filter.name + '">\
			...
			controlButtons: function(){
				...
				if(!$ul.siblings('.dt-filter-reset').length){
					aReset = '<a class="dt-filter-reset dt-filter-reset-buttons btn btn-light" atas="global__filter-panel__form_resert-filter-btn">'+_dtFilterSettings.txtResetBtn+'</a>';
