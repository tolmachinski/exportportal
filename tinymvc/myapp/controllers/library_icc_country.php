 ...
 */
class Library_Icc_Country_Controller extends TinyMVC_Controller {
    ...
    private function load_main(){
        ...
        $this->load->model('Lib_Icc_Country_Model', 'icc');
    }
    ...
    public function all(){
        ...
        checkURI($uri, array('library_icc_country', 'country', 'page'));
        ...
        $data['list_sort_by'] = array(
            'agencies-asc'  => 'Agencies name &#9650;',
            'agencies-desc' => 'Agencies name &#9660;',
            'country-asc'   => 'Country &#9650;',
        ...
        $data['link_country'] = get_dynamic_url($current_lib['link_public'] . '/' . $links_tpl_without['country']);
        list($data['page_link'], $data['get_per_p']) = explode('?', get_dynamic_url($current_lib['link_public'] . '/' . $links_tpl_without['per_p']));

        $data['count'] = $this->icc->get_icc_country_count($params);
        $data['list_icc_country'] = $this->icc->get_all_icc_country($params);
        ...
        $data['library_name']   = 'library_icc_country';
        ...
        $data['header_out_content'] = 'new/library_settings/library_icc_country/header_view';
        $data['main_content'] = 'new/library_settings/library_icc_country/index_view';
    ...
    public function get_email_by_id(){
        ...
        $record = $this->icc->get_icc_country(array('id_record' => $id_record));

        if(empty($record['email'])) {
            jsonResponse("Error: Email for this ICC Country doesn't found!");
        }
    ...
    public function administration(){
        ...
        $this->view->assign($data);
        $this->view->assign('title', 'Configs Library ICC Country');
        $this->view->display('admin/header_view');
        $this->view->display('admin/library_settings/icc_country/icc_country_view');
        $this->view->display('admin/footer_view');
    }

    public function ajax_dt_icc_country(){
        ...
        $records = $this->icc->get_all_icc_country($params);
        $records_count = $this->icc->get_icc_country_count($params);
        ...
        foreach($records as $record){
            $output['aaData'][] = array(
                'dt_id_record'  =>  $record['id_icc'] . '<div class="ml-5 w-15 pull-right"><input class="checked-element" type="checkbox" value="' . $record['id_icc'] . '">',
                'dt_country'    =>  $record['id_country'] ? $record['country'] : '---',
                'dt_agencies'   =>  $record['agencies'],
                ...
                'dt_actions'    =>  '<a class="ep-icon ep-' . ($record['is_visible'] ? 'icon_invisible' : 'icon_visible') . ' confirm-dialog" data-callback="visible_status" data-record="' . $record['id_icc'] . '" data-status="' . $record['is_visible'] . '" title="Change status record" data-message="Are you sure you want to change status of this record?" href="#" ></a>
                                    <a class="fancyboxValidateModalDT fancybox.ajax ep-icon ep-icon_pencil" href="library_icc_country/popup_forms/edit_record/'. $record['id_icc'] . '" data-title="Edit ICC Country" title="Edit this ICC Country"></a>
                                    <a class="ep-icon ep-icon_remove txt-red confirm-dialog" data-callback="remove_acc_record" data-record="' . $record['id_icc'] . '" title="Remove this ICC Country" data-message="Are you sure you want to delete this ICC Country?" href="#" ></a>',
            );
        }
        ...
        switch ($form) {
            case 'add_record':
                ...
                if(!empty($library['file_name'])){
                    ...
                    $data['config'] = $config;
                    $data['current_contoller'] = 'library_icc_country';
                }
            ...
            case 'edit_record':
                ...
                $data['record'] = $this->icc->get_icc_country(array('id_record' => $id_record));

                if(empty($data['record'])) {
                    messageInModal('Error: This library setting does not exist.');
                }

                $data['id_record'] = $data['record']['id_icc'];
            ...
            case 'update_by_country':
                $this->load->model('Country_Model', 'country');
                $data['empty_country_records'] = $this->icc->get_all_icc_country(array('country_id' => 0, 'group_by' => 'country'));
                $data['countres'] = $this->country->get_countries();
                $this->view->display('admin/library_settings/icc_country/forms/update_country', $data);
            break;
        }
    }
    ...
    public function ajax_library_operation(){
        ...
        switch($operation){
            case 'change_status':
                ...
                $exist_record = $this->icc->check_icc_country(array('id_record' => $id_record, 'visible_record' => $status_record));

                if (!$exist_record) {
                    jsonResponse('Info: The ICC Country with this id and status doesn\'t found', 'info');
                }

                if (!$this->icc->update_icc_country($id_record, array('is_visible' => !$status_record))){
            ...
            case 'save_record_manual':
                ...
                $exist_record = $this->icc->check_icc_country(array('email' => cleanInput($_POST['email'], true)));
                ...
                $insert = array(
                    ...
                    'agencies'   => cleanInput($_POST['agencies']),
                ...
                $id_record = $this->icc->set_icc_country($insert);

                if($id_record) {
                    jsonResponse('The library ICC Country have been successfully saved.', 'success');
                } else {
                    jsonResponse('Error: You cannot add library ICC Country now. Please try again later.');
                }
            break;
            ...
            case 'file_parse':
                ...
                if($delete_record && $this->icc->get_icc_country_count(array('type_record' => 0))){
                    if(!$this->icc->delete_icc_country(array('type_record' => 0))) {
                        jsonResponse('Error: You cannot remove ICC Country now.');
                    }
                }

                if(!$this->icc->set_rows_icc_country($result)) {
            ...
            case 'edit_manual_record':
                ...
                if ($this->icc->update_icc_country($id_record, $update)) {
                    jsonResponse('The ICC Country have been successfully changed.', 'success');
                } else {
                    jsonResponse('Error: You cannot edit ICC Country now. Please try again later.');
                }
            break;
            ...
            case 'remove_record':
                ...
                if(!$this->icc->check_icc_country(array('id_record' => $id_record))) {
                    jsonResponse('This ICC Country doesn\'t exist.');
                }

                if($this->icc->delete_icc_country(array('id_record' => $id_record))) {
                    jsonResponse('The ICC Country has been successfully removed.', 'success');
                } else {
                    jsonResponse('Error: You cannot remove this ICC Country now. Please try again later.');
                }
            break;
            ...
            case 'remove_records':
                ...
                if($this->icc->delete_icc_country(array('id_records' => $list_elements))) {
            ...
            case 'change_country_icc':
                ...
                if (!$this->icc->check_icc_country(array('country_name' => $contry_name_change))){
                ...
                if (!$this->icc->update_country_icc($params)){
