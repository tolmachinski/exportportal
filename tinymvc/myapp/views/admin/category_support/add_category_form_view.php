        ...
        <table cellspacing="0" cellpadding="0" class="data table-striped table-bordered w-100pr vam-table">
            <tbody>
                <tr>
                    ...
                    <td>
                        <input type="text" class="w-100pr validate[required,maxSize[50]]" name="category_name">
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        foreach($list_ep_staff as $user){?>
                            <option value="<?php echo $user['idu']?>"><?php echo $user['fname']?> <?php echo $user['lname']?> - <?php echo $user['gr_name']?></option>
                        <?php }}else{?>
...
<script>
    ...
    function modalFormCallBack(form, data_table){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo __SITE_URL?>category_support/ajax_category_support_operation/add_category_support',
            data: $form.serialize(),
            beforeSend: function () {
                showLoader($form);
            },
