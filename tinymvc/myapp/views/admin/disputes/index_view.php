...
var disputesFilters; //obj for filters
var dtDispute; //obj of datatable
$(document).ready(function(){
	...
	dtDispute = $('#dtDispute').dataTable( {
		...
		"bServerSide": true,
		"bSortCellsTop": true,
		"sAjaxSource": "<?php echo __SITE_URL?>dispute/ajax_admin_disputes_dt/<?php echo $upload_folder?>",
		"sServerMethod": "POST",
		"iDisplayLength": 10,
		"aLengthMenu": [
		    [10, 25, 50, 100, -1],
...
var delete_my_image = function($opener){
	var $this = $($opener);
	var $image_elem = $this.closest('.one-image').find('img');
	var file_name = $image_elem.data('file');
	var dispute_id = $image_elem.data('id');
	$.ajax({
		url: '/dispute/ajax_operation/delete_photo/'+dispute_id+'/'+file_name,
		dataType: "JSON",
		success: function(resp){
			systemMessages( resp.message, 'message-' + resp.mess_type );
			if(resp.mess_type == 'success'){
				$this.closest('.one-image').remove();
			}
		}
	});
}

var delete_my_video = function($opener){
	var $this = $($opener);
	$.ajax({
		url: '/dispute/ajax_operation/delete_video/'+$this.data('id_dispute')+'/'+$this.data('id'),
		dataType: "JSON",
		success: function(resp){
			systemMessages( resp.message, 'message-' + resp.mess_type );
			if(resp.mess_type == 'success'){
...
<div class="row">
    <div class="col-xs-12">
		<?php tmvc::instance()->controller->view->display('admin/disputes/filter_panel_view')?>
		<div class="titlehdr h-30"><span>Disputes</span></div>
		...
		<ul class="menu-level3 mb-10 clearfix">
			...
			<?php foreach($statuses as $status_key => $status){?>
				<li class="<?php echo active($status_key, 'init');?>">
