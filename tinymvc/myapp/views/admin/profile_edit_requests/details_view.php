<div class="wr-modal-b">
    <div class="modal-b__content pb-0 w-900" id="preview-profile-edit-request--wrapper">
        <div class="row">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="w-25pr tac">
                                <strong>Type</strong>
                                <div>
                                    <span class="fs-12 label label-info">
                                        Profile
                                    </span>
                                </div>
                            </td>
            ...
            <div class="col-xs-12">
                ...
                <?php } else { ?>
                    ...
                    <div class="form-control">
                        User <a href="<?php echo cleanOutput($profileUrl); ?>" target="_blank"><?php echo cleanOutput($userName); ?></a> requested profile update.
                    </div>
                <?php } ?>
            </div>

            <?php if (!empty($userDiff)) { ?>
                <div class="col-xs-12">
                    <table
                        cellspacing="0"
                        cellpadding="0"
                    ...
                    >
                        ...
                        <tbody class="tabMessage">
                            <?php foreach ($userDiff as $key => list($current, $new)) { ?>
                                <tr>
    ...
    <div class="modal-b__btns clearfix mnh-60">
        <?php if ($isPending) { ?>
            <button
                id="preview-profile-edit-request--action--accept"
                type="button"
            ...
            <button
                id="preview-profile-edit-request--action--decline"
                type="button"
...
<script><?php echo getPublicScriptContent('plug_admin/js/profile_edit_requests/details.js', true); ?></script>
<script>
    $(function () {
		if (!('ProfileEditRequestDetailsModule' in window)) {
			if (__debug_mode) {
				console.error(new SyntaxError("'ProfileEditRequestDetailsModule' must be defined"))
			}

			return;
        }

        ProfileEditRequestDetailsModule.default(
            <?php echo json_encode([
                'acceptUrl'              => $acceptUrl,
                'acceptButton'           => '#preview-profile-edit-request--action--accept',
                'detailsWrapper'         => '#preview-profile-edit-request--wrapper',
