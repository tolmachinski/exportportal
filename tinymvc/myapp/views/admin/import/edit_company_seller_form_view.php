<?php $company_data = json_decode($company['import_data'], true); ?>
...
<form class="relative-b validateModal">
        ...
        <table cellspacing="0" cellpadding="0" class="data table-striped w-100pr mt-15 vam-table">
            <tbody>
                <tr>
                    ...
                    <td colspan="2">
                        <input type="text" name="company_name" class="validate[required,maxSize[100]] w-100pr" value="<?php echo $company_data['company_name'];?>" placeholder="Company name"/>
                    </td>
                </tr>
                <tr>
                    <td class="w-100">Seller name</td>
                    <td>
                        <input type="text" name="fname" class="validate[required,maxSize[50]] w-100pr" value="<?php echo $company_data['user_fname'];?>" placeholder="First name"/>
                    </td>
                    <td>
                        <input type="text" name="lname" class="validate[required,maxSize[50]] w-100pr" value="<?php echo $company_data['user_lname'];?>" placeholder="Last name"/>
                    </td>
                </tr>
                <tr>
                    <td class="w-100">Email</td>
                    <td colspan="2">
                        <input type="text" name="email" class="validate[required,maxSize[100],custom[noWhitespaces],custom[emailWithWhitespaces]] w-100pr" value="<?php echo $company_data['email'];?>" placeholder="Email"/>
                    </td>
                </tr>
                <tr>
                            ...
                            <?php foreach($groups as $group){?>
                                <option
                                    ...
                                    <?php if($user['status'] != 'new'){?>
                                        <?php echo selected($company_data['group'], $group['idgroup']);?>
                                        <?php if($company_data['group'] == $group['idgroup']) $group_name = $group['gr_name'];?>
                                    <?php }?>
                ...
                <tr>
                    ...
                    <td>
                        <select name="port_country" id="country" class="w-100pr validate[required]">
                            ...
                            <?php $country_name = $company_data['country'];?>

                            <?php foreach($port_country as $mconutry){ ?>
                                <option value='<?php echo $mconutry['id']?>'
                                <?php if($user['status'] != 'new'){?>
                                    <?php echo selected($company_data['id_country'], $mconutry['id']); ?>
                                    <?php if($company_data['id_country'] == $mconutry['id']) $country_name = $mconutry['country'];?>
                                <?php }?>
                ...
                <tr>
                    ...
                    <td id="user_state">
                        <?php $state_name = $company_data['state'];?>
                        <?php if($user['status'] == 'new'){?>
                        ...
                        <?php } else{?>
                            <select name="states" class="w-100pr validate[required]" id="states">
                                ...
                                <?php foreach($states as $state){?>
                                    <option id="<?php echo $state['id'];?>" value="<?php echo $state['id'];?>"
                                        <?php echo selected($company_data['id_state'], $state['id']);?>
                                        <?php if($company_data['id_state'] == $state['id']) $state_name = $state['state'];?>
                ...
                <tr>
                    ...
                    <td colspan="2">
                        <input type="text" name="address" class="validate[required,minSize[2],maxSize[255]] w-100pr" value="<?php echo $company_data['address'];?>" placeholder="Address"/>
                    </td>
                </tr>
                <tr>
                    <td class="w-100">Zip</td>
                    <td colspan="2">
                        <input type="text" name="zip" class=" w-100pr validate[required,custom[zip_code],maxSize[20]]" maxlength="20" value="<?php echo $company_data['zip'];?>" placeholder="Zip"/>
                    </td>
                </tr>
                <tr>
                    <td class="w-100">Phone</td>
                    <td colspan="2">
                        <input type="text" name="phone" class="validate[required,minSize[12],maxSize[25]] w-100pr" value="<?php echo $company_data['phone'];?>" placeholder="Phone"/>
                    </td>
                </tr>
                <tr>
                    <td class="w-100">Company logo</td>
                    <td colspan="2">
                        <input type="text" name="logo" class="validate[required] w-100pr" value="<?php echo $company_data['logo'];?>" placeholder="Company logo"/>
                    </td>
                </tr>
            </tbody>
        </table>
	...
	<div class="wr-form-btns clearfix">
	    ...
	    <input type="hidden" name="id_import" value="<?php echo $company['id'];?>">
