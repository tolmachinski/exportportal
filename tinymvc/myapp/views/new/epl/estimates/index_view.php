...
<div class="container-center dashboard-container">
    <div class="dashboard-line">
        <h1 class="dashboard-line__ttl">Shipping estimates</h1>

        <div class="dashboard-line__actions">
            <a class="btn btn-dark btn-filter fancybox btn-counter" data-fancybox-href="#dtfilter-hidden" data-title="<?php echo translate("general_dt_filters_modal_title"); ?>" data-mw="740" title="<?php echo translate("general_dt_filters_button_title"); ?>">
    ...
    <table class="main-data-table" id="dtShippingRequests">
        <thead>
            <tr>
                ...
                <th class="countdown_dt"><?php echo translate("shipping_estimates_dashboard_dt_estimate_status_expired_label_text"); ?></th>
...
<script type="application/javascript">
    $(function() {
        ...
        var onDeleteFilters = function(filter) {
            ...
            if (filter.name === "from_country") {
                $('.dtfilter-popup a[data-parent="from_state"]').click();
            }
            if (filter.name === "from_state") {
                $('.dtfilter-popup a[data-parent="from_city"]').click();
            }
            ...
            if (filter.name === 'group_key') {
                var url = __current_sub_domain_url + 'shippers/estimates_requests';
                if (
        ...
        var onDeleteEstimate = function(caller) {
            ...
            var url = __current_sub_domain_url + 'shippers/ajax_shippers_operation/delete_estimate';
            ...
            if (null !== estimate) {
                $.post(url, {
                    id: estimate
                }, null, 'json').done(onRequestSuccess).fail(onRequestError);
            }
        };
        var onSaveEstimate = function(caller) {
            var button = $(caller);
            var estimate = button.data('estimate') || null;
            var url = __current_sub_domain_url + 'shippers/ajax_shippers_operation/save_for_order';
            var onRequestSuccess = function(response) {
                systemMessages(response.message, response.mess_type);
                if ('success' === response.mess_type) {
        ...
        var onRenewEstimate = function(caller) {
            var button = $(caller);
            var estimate = button.data('estimate') || null;
            var url = __current_sub_domain_url + 'shippers/ajax_shippers_operation/renew_estimate';
            var onRequestSuccess = function(response) {
                systemMessages(response.message, response.mess_type);
                if ('success' === response.mess_type) {
                    dtShippingRequests.fnDraw(false);
        ...
        var onChangeCountry = function(statesList, citiesList) {
            return function(event) {
                ...
                if (statesList.length) {
                    ...
                    if (myFilters) {
                        myFilters.removeFilter(statesList.prop('name'), false);
                        myFilters.removeFilter(citiesList.prop('name'), false);
                    }

                    $.post(__current_sub_domain_url + 'location/ajax_get_states', {
                            country: country
                        }, null, 'json')
        ...
        var onChangeState = function(citiesList) {
            return function(event) {
                ...
                citiesList.val(null).prop('disabled', null === region);
                citiesList.children().not(citiesPlaceholder).remove();
                if (myFilters) {
                    myFilters.removeFilter(citiesList.prop('name'), false);
                }
        ...
        var datagridOptions = {
            ...
            bProcessing: false,
            bServerSide: true,
            sAjaxSource: location.origin + '/shippers/ajax_estimates_dt',
            aoColumnDefs: [{
                    sClass: "mw-300",
                    aTargets: ['estimate_dt'],
                    mData: "estimate",
                    bSortable: true
                },
                {
                    sClass: "dn-xl",
                    aTargets: ['destination_dt'],
                    mData: "destination",
                    bSortable: false
                },
                {
