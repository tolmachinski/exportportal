<div class="js-modal-flex wr-modal-flex inputs-40">
	<form class="modal-flex__form validateModal" data-callback="start_order_by_producing_request">
...
<script>
	...
	function start_order_by_producing_request(form){
		var $form = $(form);
		var $wrform = $form.closest('.js-modal-flex');
		$form.find('button[type="submit"]').prop('disabled', true);

		$.ajax({
			url: __site_url + 'po/ajax_po_operation/create_order',
			type: 'POST',
			data:  $form.serialize(),
			dataType: 'json',
			beforeSend: function(){
			...
			success: function(data){
				...
				if(data.mess_type == 'success'){
					current_status = 'order_initiated';
					current_page = 1;
