<div class="js-modal-flex wr-modal-flex inputs-40">
	<form class="modal-flex__form validateModal" data-callback="start_order_by_offer">
		<div class="modal-flex__content">
            ...
            <input type="hidden" name="offer" value="<?php echo $id_offer;?>"/>
		</div>
...
<script>
	...
	function start_order_by_offer(form){
		var $form = $(form);
		var $wrform = $form.closest('.js-modal-flex');
		$form.find('button[type="submit"]').prop('disabled', true);

		$.ajax({
			url: '<?php echo __SITE_URL; ?>offers/ajax_offers_operation/create_order',
			type: 'POST',
			data:  $form.serialize(),
			dataType: 'json',
			beforeSend: function(){
