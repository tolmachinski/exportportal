<div class="js-modal-flex wr-modal-flex inputs-40">
	<form class="modal-flex__form validateModal" data-callback="start_order_by_inquiry">
		...
		<div class="modal-flex__btns">
			<input type="hidden" name="id_inquiry" value="<?php echo $id_inquiry;?>"/>
...
<script>
	...
	function start_order_by_inquiry(form){
		var $form = $(form);
		var $wrform = $form.closest('.js-modal-flex');
		$form.find('button[type="submit"]').prop('disabled', true);

		$.ajax({
			url: '<?php echo __SITE_URL; ?>inquiry/ajax_inquiry_operation/create_order',
			type: 'POST',
			data:  $form.serialize(),
			dataType: 'json',
			beforeSend: function(){
